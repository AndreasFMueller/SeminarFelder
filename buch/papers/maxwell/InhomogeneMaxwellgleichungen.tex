...
Da wir uns im vierdimensionalen Minkowski-Raum befinden, wird durch Anwendung des Hodge-Operators auf die 2-Form $F$ wieder eine 2-Form.
...
\begin{align*}
	...
	&\phantom{=} + \, B_3 \, \, {\ast}(dx^1 \wedge dx^2) - \, B_2 \, \, {\ast}(dx^1 \wedge dx^3) + B_1 \, {\ast}(dx^2 \wedge dx^3).
\intertext{Die Hodge-Duale können aus der Tabelle \ref{maxwell:section:teil1:Hodge-Tabelle} entnommen werden und wir erhalten}
	\ast F &=
	\phantom{+}  \frac{E_{1}}{c} \, dx^2 \wedge dx^3 - \frac{E_{2}}{c} \, dx^1 \wedge dx^3 + \frac{E_{3}}{c} \, dx^1 \wedge dx^2 \\
	 &\phantom{=}+ \, B_3 \, \, dx^0 \wedge dx^3 + \, B_2 \, \, dx^0 \wedge dx^2 + \, B_1 \, \, dx^0 \wedge dx^1.
\end{align*}
...
\begin{align*}
	...
	\\
	&\phantom{=}- \frac{1}{c} \frac{\partial E_2}{\partial x^2} \, dx^2 \wedge dx^1 \wedge dx^3
	...
	\\
	&\phantom{=}+\phantom{\frac{1}{c}} \frac{\partial B_3}{\partial x^1} \, dx^1 \wedge dx^0 \wedge dx^3
	...
	&\phantom{=}+\phantom{\frac{1}{c}} \frac{\partial B_2}{\partial x^3} \, dx^3 \wedge dx^0 \wedge dx^2
...
\begin{equation*}
	c = \frac{1}{\!\sqrt{\varepsilon_0 \mu_0\mathstrut}}
\end{equation*}
...
\begin{align*}
	\ast d {\ast} F
	& = \mu_0 \left( -\frac{\rho}{\varepsilon_0 \mu_0}\,dt + J_1\, dx + J_2\, dy + J_3\, dz \right)\\[1ex]
...
\begin{equation}
	\ast d {\ast} F = \mu_0 J.
\end{equation}
Zusätzlich ist die Formulierung kovariant und behält somit ihre Form bei Koordinatentransformationen.
\begin{comment}
