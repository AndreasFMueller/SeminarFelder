...
\kopfrechts{Geschlossene 1-Formen und Wegunabhängigkeit}%
...
dx^i.
\]
In der Summe tritt jedes Paar von verschiedenen Indizes genau zweimal
in verschiedener Reihenfolge auf, wegen
$dx^i\wedge dx^k = - dx^k \wedge dx^i$
kann man
\begin{align}
d\alpha
&=
...
in der differenzierbaren Manigfaltigkeit mit den konstanten Endpunkten
...
kann man den Äquator über den Nordpol oder Südpol zusammenziehen.
...
Von den Weg\-in\-te\-gra\-len bleiben daher nur
\[
0
=
\int_{\partial[0,1]^2} (TH)^*(\alpha)
...
\dot{\gamma}(1) = e_i
\]
der $i$-te Standardbasisvektor ist.
...
In der Physik wird es im Falle des Gravitationsfeldes meistens noch
durch die Masse und im Falle des elektrostatischen Potentials durch
die Ladung geteilt,
