...
\begin{scope}[xshift=-3.2cm,yshift=-3.8cm]
	...
	\draw[line width=0.2pt] ({-\l+\rechts},-\l) -- 
	    node[below=1.5cm,anchor=north west,xshift=-2mm]{$A_k = \left\{ x \mid T(x) \in B_k \right\}$} 
		({\l+\rechts},\l);
...
\begin{scope}[xshift=-3.2cm]
	\ifthenelse{\boolean{withSpace}}{
		\fill[color=gray!20] (-2.9,0) rectangle (2.9,3.2);
		\node[color=white] at (-2.3,2.5) [scale=3] {$X$};
	}{}
	...
	\node[darkred] at (-1.7,1.4) {$\mu(x)\mathstrut$};
...
\begin{scope}[xshift=3.2cm]
	\ifthenelse{\boolean{withSpace}}{
		\fill[color=gray!20] (-2.9,0) rectangle (2.9,3.2);
		\node[color=white] at (0,2.5) [scale=3] {$Y$};
	}{}
