...
\begin{equation}
    \quad
    x_g \;=\; \int_{0}^{z_a} \frac{c(\zeta)}{\sqrt{V^2 - c(\zeta)^2}}\;\, d\zeta,
...
Wendepunkt mit Bodenschatten. Diese Szenarien werden in den Abbildungen
\ref{fig:schall:norm-lage}, \ref{fig:schall:inv-lage} und \ref{fig:schall:norm-vs-inv-lage}
visualisiert und im Folgenden beschrieben.
...
\begin{equation}
    x_{\mathrm{turn}}
    \;=\; \int_{z_\ast}^{z_a} \frac{c(\zeta)}{\sqrt{V^2 - c(\zeta)^2}}\;d\zeta,
    \label{eq:x-turn}
\end{equation}
...
Letzteres führt zu einer \emph{breiteren Lärmwahrnehmung} am Boden.